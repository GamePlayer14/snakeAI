...
game = SnakeGame(BOARD_SIZE, render=True, tiles=tiles, master=root)
bind_controls(root, game)

# Setup autoencoder
model = OneLayerAutoencoder().to(device)
optimizer = optim.Adam(model.parameters(), lr=0.001)
loss_fn = nn.MSELoss()

# Training loop tied to GUI loop
def loop():
    game.step()
    input_tensor = extract_spatial(game).to(device)
    ...
    if loop.step_count % 50 == 0:
        print(f"Step {loop.step_count} | Loss: {loss.item():.5f}")

    if loss.item() < 0.0005:
        torch.save(model.encoder.state_dict(), "saved_encoder.pth")
        print("✅ Encoder saved to saved_encoder.pth")
...
# Start everything
game.reset()
tk.Button(root, text="Reset", command=game.reset).pack()
loop()
root.mainloop()
